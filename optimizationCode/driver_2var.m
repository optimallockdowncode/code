...
%
% NOTE: also edit lines 42-58 below so that 
%           the variables you want receives these values
INDEP_VARIABLE1 = "\rho";
INDEP_VARIABLE2 = "VSL";
vals1 = 0.1:0.2:0.9;
vals2 = 15*(1:10);
...
for count1 = 1:length(vals1)
    ...
    T = 364;
    dt = 1; % highest resolution dt = 1
            % for faster but rougher simulations, can increase this
    t = 0:dt:T;
    ...
    Constants = ...
        ...
        'beta', 0.24*m;     %transmission/contact rate
        'sig',0.15;      %transfer from E to I
        'gamma',gamma;      %recovery rate
        'theta',0.25;     %mixing rate for quarantined population
        'tau',0;      %endemic (loss of immunity) rate
        'rho', vals1(count1); %0.7;       %work from home percentage
        'kappa',0.05*gamma; % 0.05*gamma%excess deaths
        'c1',1e-4;
        'c2',1e-4;
        'a',0.5;   %weight on economic vs healthcare objective
        'vsl',vals2(count2);
        'b1',0.008; %baseline 0.008; 
        'b2',13.5; %baseline 13.5
        'b3',0.016; %baseline 0.016
        };
    ...
    Intermediates = ...
        ...
        'Ts = (S+E+IA+RA+Sq+Eq+IAq+RAq)*tbar';
        'LTs= Ts*50/200'; %50 is test cost 200 is daily earning        
        'Tr = mu*Ts';
        'LTr=Tr/20';
        'N1 = S + E + IA + IS + J + RA + RS';
        'Nq = Sq + Eq + IAq + ISq + Jq + RAq + RSq';
        'Wf = N1-(1-rho)*J-IS+rho*(Nq - ISq) - LTs - LTr';
    ...
    Equations = ...
        ...
        '$E  = lam * S - (sig + tbar*(0.5 + mu)) * E - p * E + pq * Eq';
        '$Eq = theta * lam * Sq - (sig + tbar*(0.5 + mu)) * Eq + p * E - pq * Eq';
        '$IA = 0.5 * sig * E  - (gamma + tbar*(0.5 + mu)) * IA  - p * IA + pq * IAq';
        '$IAq= 0.5 * sig * Eq - (gamma + tbar*(0.5 + mu)) * IAq + p * IA - pq * IAq';
        '$IS = 0.5 * sig * E - (gamma + drate) * IS - p * IS + pq * ISq';
        '$ISq= 0.5 * sig * Eq - (gamma + drate) * ISq + p * IS - pq * ISq';
        '$J  = tbar*(0.5 + mu) * (IA + E) - gamma * J - p * J + pq * Jq';
        '$Jq = tbar*(0.5 + mu) * (IAq + Eq) - gamma * Jq + p * J - pq * Jq';
        '$D = drate*(IS + ISq)';
        ...
        '$Production  = a*Wf/(364*N) - (1-a)*drate*(IS + ISq)*vsl/N - c1*(p^2+pq^2)-c2*(mu^2+tbar^2)';
        'Nq <= 0.7*N';
        % logistic testing function
